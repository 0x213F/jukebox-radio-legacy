...
class Record(BaseModel):
    ...
    def delete(self):
        ...
        now = datetime.now()
        ongoing_stream = self.now_playing_at_streams.filter(
            record_terminates_at__gte=now
        )
    ...
    def duration_ms(self):
        ...
        track_durations_ms = (
            TrackListing.objects.filter(record=self)
            .order_by("number")
    ...
    async def spotify_uris(self):
        ret_val = await database_sync_to_async(
            record.tracks_through.order_by("number").values_list
